...
void delet(char *st_name) {
    ...
    sort_secindex();
    for (int i = 0; i < no; i++) {
        f1 << rec[i].ind << "|" << rec[i].usn << "|" << rec[i].name << "|"
...
int main() {
    ...
    int choice;
    char rt_usn[20], st_name[20], st_usn[20], ind[5], name[20], age[5], sem[5],
        branch[5], usn[20];
    int i, flag1, flag;
...

    {
        ...
        case 1: {
            ...
            for (i = 0; i < no; i++)
                cout << sec[i].name << " " << sec[i].usn << " " << sec[i].ind
                     << endl;
